...
-------------------------------------------------------------------------------
-- | Exercise a.
vector :: (Integral b) => b -> a -> Vector a
...
get i (V tam arb)
      | i>tam-1 = error "Index out"
      | otherwise = buscaElem i arb tam

buscaElem i (Unif x) tam = x
buscaElem i (Node izq der) tam
      | i <= ((div tam 2) - 1) = buscaElem i izq (div tam 2)
      | otherwise = buscaElem (i-(div tam 2)) der (div tam 2)

-------------------------------------------------------------------------------
-- | Exercise e.

set i x (V tam (Unif c))
      | tam == 1        = (V tam (Unif x))
      | i <= (nTam - 1) = (V tam (simplify (getArbol (set i x (V nTam (Unif c)))) (Unif c)))
      | otherwise       = (V tam (simplify (Unif c) (getArbol (set (i-nTam) x (V nTam (Unif c))))))
          where 
            nTam = (div tam 2)

set i x (V tam (Node izq der))
      | i <= (nTam - 1) = (V tam (simplify (getArbol (set i x (V nTam izq))) der))
      | otherwise       = (V tam (simplify izq (getArbol (set (i-nTam) x (V nTam der)))))
        where 
          nTam = div tam 2

getArbol (V x arb) = arb
...
mapVector f (V tam (Unif x)) = (V tam (Unif (f x)))
mapVector f (V tam (Node izq der)) = (V tam (simplify (getArbol (mapVector f (V nTam izq))) (getArbol (mapVector f (V nTam der)))))
    where 
      nTam = div tam 2
-------------------------------------------------------------------------------
-- | Exercise h.
