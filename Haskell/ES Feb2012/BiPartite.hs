...
aux g dict stack
   ...
   | not (colored v)       = aux g nDict nStack
   | (colored v) && c /= c'= Nothing
...
--
  where
     ...
     (v,c)              = S.top stack
     nColor             = nextColor c
     nDict              = D.insert v c dict
     nStack             = (pushAll stack [ (v',nColor) | v' <- (successors g v), not (colored v')])
     Just c'            = D.valueOf v dict
--
