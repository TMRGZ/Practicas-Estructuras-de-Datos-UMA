...
delete_insert_1 x s     = delete x (insert x s)            == insert x s
...
union (Node x ox s) (Node y oy t)
  ...
  | y < x     = (Node y oy (Node x ox (union s t)))
