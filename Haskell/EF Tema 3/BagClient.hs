...
bag2List bag = foldBag (\e oe s -> (e,occurrences e bag):s) [] bag
...
contains x bag = foldBag (\e oe s -> (occurrences x bag)>0) False bag
...
maxOcurrences bag = foldBag (\e oe s -> max (occurrences e bag) s) 0 bag
