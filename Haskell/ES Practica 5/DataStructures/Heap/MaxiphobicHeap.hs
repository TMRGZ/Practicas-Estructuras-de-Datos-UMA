...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = if (size lh) < (size rh) then Node x (sz+sz') rh (merge lh h') else Node x (sz+sz') lh (merge rh h')
 | otherwise       = if (size lh') < (size rh') then Node x' (sz+sz') rh' (merge lh' h) else Node x' (sz+sz') lh' (merge rh' h)
----------------------------------------------------------
-- ^^^^^^^^^^^^^^-- SOLO TOCAR ARRIBA ^^^^^^^^^^^ ---------
----------------------------------------------------------
