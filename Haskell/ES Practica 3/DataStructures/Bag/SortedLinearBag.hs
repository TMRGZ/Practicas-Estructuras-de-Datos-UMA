...
insert x (Node y oy s)
    | x == y    = (Node y (oy+1) s)         -- Si esta ya en la bolsa le sumo 1 al contador
    | x > y     = Node y oy (insert x s)    -- Si lo que meto es mayor que lo que veo, me muevo al siguiente nodo
    | otherwise = Node x 1 (Node y oy s)    -- Si encuentro una buena posicion, lo meto con contador = 1
...
delete x (Node y oy s)
    | occurrences x (Node y oy s) == 0          = Node y oy s
    | x == y = if occurrences x (Node y oy s) > 1 then Node y (oy-1) s else s
...
union (Node x ox s) (Node y oy t)
  ...
  | otherwise = if ox>1 then insert x (union (Node x (ox-1) s) ((Node y oy t))) else insert x (union (s) ((Node y oy t)))
...
intersection (Node x ox s) (Node y oy t)
  | x == y    = if ox < oy then Node x ox (intersection s t) else Node x oy (intersection s t)
...
difference (Node x ox s) (Node y oy t)
  | x == y = if ox > oy then difference (Node x (ox-oy) s) t else difference s t
