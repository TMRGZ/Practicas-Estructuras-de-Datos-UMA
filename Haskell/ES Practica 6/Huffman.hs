...
huffmanLeaves s = foldr (\(ek, ev) s -> (PQ.enqueue (WLeaf ek ev) s)) PQ.empty xs
  where
    xs = (D.keysValues (weights s))
...
huffmanTree s
  | (D.size (weights s)) > 1 = unirArboles (huffmanLeaves s)
  | otherwise = error "Menos de dos simbolos distintos"


unirArboles :: PQ.PQueue (WLeafTree Char) -> WLeafTree Char
unirArboles q 
  | PQ.isEmpty (PQ.dequeue q)  = PQ.first q
  | otherwise       = unirArboles ((PQ.enqueue (merge a1 a2)) (PQ.dequeue (PQ.dequeue q)))
    where
      a1 = PQ.first q
      a2 = PQ.first (PQ.dequeue q)
...
huffmanCode (WNode izq der _) = (joinDics (prefixWith 0 (huffmanCode izq)) (prefixWith 1 (huffmanCode der)))
...
encode [] d = []
encode (x:xs) d = sacarMaybe(D.valueOf x d) ++ (encode xs d)
...
takeSymbol l (WLeaf v _) = (v, l)
takeSymbol (x:xs) (WNode izq der _)
      | x == 0    = takeSymbol xs izq
      | otherwise = takeSymbol xs der
...
decode [] arb = []
decode l arb = letraAct : (decode listaRes arb)
      where
        (letraAct, listaRes) = (takeSymbol l arb)
