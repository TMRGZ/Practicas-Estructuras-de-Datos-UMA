...
-- primAux g ts  rs st : 
      --ts son los nodos visitados  que forman parte del árbol, 
      --rs el resto de nodos por visitar y st almacena el árbol de expansión (lista de aristas con peso) computado hasta el momento
...
primAux g ts rs st = primAux g (y:ts) (delete y rs) (WE x p y:st)
   where
      ...
      aristas = [ WE x p y | x <- ts, (y, p) <- successors g x, elem y rs ]
      -- de todas las aristas calculadas en el paso anterior encuentra la de menor peso
      (WE x p y) = minimumBy comparaArcos aristas
        where comparaArcos (WE _ p1 _) (WE _ p2 _) = compare p1 p2
