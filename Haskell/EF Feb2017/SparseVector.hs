...
module SparseVector( Vector, vector, size, get, set, mapVector, filterVector, depthOf, pretty) where
...
-------------------------------------------------------------------------------
-- | Exercise c.
simplify (Node x y) (Node x' y') = Node (Node x y) (Node x' y')
simplify (Unif x) (Unif x')
    | x == x'   = Unif x
    | otherwise = Node (Unif x) (Unif x')
...
get i (V i' (Unif c)) = c
get i (V i' (Node lt rt))
    | i < (div i' 2)    = get i (V (div i' 2) lt)
    | otherwise         = get (i - (div i' 2)) (V (div i' 2) rt)

-------------------------------------------------------------------------------
-- | Exercise e.

set index valor (V n (Unif y))
    | n == 1                = V n (Unif valor)
    | index < div n 2       = V n (simplify (arbolV (set index valor (V (div n 2) (Unif y)))) (Unif y))
    | otherwise             = V n (simplify (Unif y) (arbolV(set (index - (div n 2)) valor (V (div n 2) (Unif y)))))
set index valor (V n (Node lt rt))
    | index < div n 2       = V n (simplify (arbolV (set index valor (V (div n 2) lt))) rt)
    | otherwise             = V n (simplify lt (arbolV(set (index - (div n 2)) valor (V (div n 2) rt))))
...
mapVector f (V n (Unif y))      = V n (Unif (f y))
mapVector f (V n (Node lt rt))  = V n (simplify (arbolV (mapVector f (V (div n 2) lt))) (arbolV (mapVector f (V (div n 2) rt))))
